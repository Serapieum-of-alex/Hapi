...
data = [15.999737471905252, 16.105716234887431, 17.947809230275304, 16.147752064149291, 15.991427126788327,
         16.687542227378565, 17.125139229445359, 19.39645340792385, 16.837044960487795, 15.804473320190725,
         16.018569387471025, 16.600876724289019, 16.161306985203151, 17.338636901595873, 18.477371969176406,
...
"""
threshold = 17
Param_dist = Gdist.EstimateParameter(method='optimization', ObjFunc=Gumbel.ObjectiveFn,
                                     threshold= threshold)
print(Param_dist)
loc = Param_dist[0]
scale = Param_dist[1]
...
upper, lower = Gevdist.ConfidenceInterval(shape, loc, scale, F=cdf_Weibul, alpha=0.1,
                                          statfunction=func,n_samples=len(data))

#%%
"""
calculate the confidence interval using the boot strap method directly
"""
CI = ConfidenceInterval.BootStrap(data, statfunction=func, gevfit=Param_dist,
                                  n_samples=len(data), F=cdf_Weibul)
LB     = CI["LB"]
UB     = CI["UB"]
#%%
