...
This Code has been implemented by Mostafa Farrag as part of Master thesis "Exploring spatial-temporal
simulation of catchment response based on Dynamic weighting of hydrological models"at IHE-Delft, NL.
this code implements the HBV-96 version, as described in Lindstrom et al (1997) with some changes in 
soil subroutine 

@author: Mostafa Farrag (moah.farag@gmail.com)
-------
10-04-2018 - V_0.0 - First implementation
"""
...
#_____________________________________________________________________________________________
"""
def muskingum_routing(inflow,Qinitial,k,x,dt):
# =============================================================================
#     inputs:
#            1- inflow
...
#    y_ext, x_ext = mask.shape # shape of the fpl raster (rows, columns)------------ should change rows are y and columns are x
    ...
    if len(sp_pars.shape) == 1:  # Checking for distributed pars 
        # if parameters array shape is 1D(only one set for all cells)
        # assign the maxbas value
#        if len(p2) > 2:  # Forcing maxbas to be predefined
#            maxbas = p2[2]  # maxbas is stored in p2 third element
...
#            maxbas = 1
        ...
        sp_pars = np.tile(sp_pars, [x_ext, y_ext, 1]) 
#        sp_pars = np.tile(sp_pars, [y_ext, x_ext, 1]) 
        # repeat all parameters for all cells
        # sp_pars[rows,cols,parameters no]
...
#            maxbas = 1
    ...
    dist_map[mask == no_val] = np.nan # replace novalue cells by nan
    
    # normalize maxbas values between 1 hr (min) and given MAXBAS (max) based on the fpl
    # shortest flow path length >> maxbas= 1
    # longest flow path length >> maxbas= maxbas
...
#    resize_fun = lambda x: np.round(((((x - min_dist)/(max_dist - min_dist))*(1*maxbas - 1)) + 1), 0)
    resize_fun = lambda x: ((((x - min_dist)/(max_dist - min_dist))*(1*maxbas - 1)) + 1)
...
#                # append column after column in the same row -----------------
                ...
                # routing lower zone flow (mm/hr not m3/sec) didn't multiplied by area
#                q_lzi.append(_routing(q_lz_temp, maxbas_map[x,y])) 
                q_lzi.append(q_lz_temp)
                # k*(UZ_int_3)**(1+alpha)
                q_uz_temp = np.array(sp_pars[x, y, 5])*(np.power(_uzg, (1.0 + sp_pars[x, y, 7])))
                
                # routing upper zone flow (mm/hr not m3/sec) didn't multiplied by area
...
#    q_lz1 = np.array([np.nansum((q_lz[:,:,i]*px_area*area_coef)/(p2[0]*3.6)) for i in xrange(n_steps)])
#    q_uz1 = np.array([np.nansum((q_uz[:,:,i]*px_area*area_coef)/(p2[0]*3.6)) for i in xrange(n_steps)])
#    q_out =q_lz1+q_uz1
    
    q_lz1 = np.array([np.nanmean(q_lz[:,:,i]) for i in range(n_steps)]) # average of all cells (not routed mm/timestep)
    q_uz1 = np.array([np.nanmean(q_uz[:,:,i]) for i in range(n_steps)]) # average of all cells (routed mm/timestep)
    
    q_out = (q_lz1 + q_uz1) * p2[1]/ (p2[0]*3.6) # generation
    
    return q_out, st,q_uz,q_lz1
"""
#_____________________________________________________________________________________________
#_____________________________________________________________________________________________
"""
def distributed_totally(flp, sp_prec, sp_et, sp_temp, sp_pars, p2, init_st=None, 
                ll_temp=None, q_0=None):
    '''
    Make spatially distributed HBV in the SM and UZ
...
#_____________________________________________________________________________________________
"""
def distributed_new_model(lakecell,q_lake,DEM,flow_acc,flow_acc_plan, sp_prec, sp_et, sp_temp, sp_pars, p2, init_st=None, 
                ll_temp=None, q_0=None):
    '''
    Make spatially distributed HBV in the SM and UZ
    interacting cells 
...
# =============================================================================
    # old
    # routing all cells with thier k & x
    for x in range(x_ext): # no of rows
        for y in range(y_ext): # no of columns
            if mask [x, y] != no_val: 
                q_uz_routed[x,y,:]=muskingum_routing(q_uz[x,y,:],q_uz[x,y,0],sp_pars[x,y,10],sp_pars[x,y,11],p2[0])
# =============================================================================
#    # new
...
#                q_uz_routed[x,y,:]=q_uz[x,y,:]        
    ...
    q_lake=muskingum_routing(q_lake,q_lake[0],sp_pars[lakecell[0],lakecell[1],10],sp_pars[lakecell[0],lakecell[1],11],p2[0])
    q_lake=np.append(q_lake,q_lake[-1])
    # both lake & Quz are in m3/s
# =============================================================================
    #old
    q_uz_routed[lakecell[0],lakecell[1],:]=q_uz_routed[lakecell[0],lakecell[1],:]+q_lake
# =============================================================================
    #new
            ...
            for y in range(y_ext): # no of columns
                    ...
                    if mask [x, y] != no_val and flow_acc_plan[x, y]==no_cells[j]:
                        ...
                        for i in range(len(flow_acc[str(x)+","+str(y)])): #  no_cells[j]
                            ...
                            q_r=q_r+q_uz_routed[x_ind,y_ind,:]
                            # route the us Q with the k & s of the current cell
                        q=muskingum_routing(q_r,q_r[0],sp_pars[x,y,10],sp_pars[x,y,11],p2[0])
                        q_uz_routed[x,y,:]=q_uz_routed[x,y,:]+q
# =============================================================================
#    # new
#    for j in range(1,len(no_cells)): #2):#
#        for x in range(x_ext): # no of rows
...
#                            q_r=q_r+muskingum_routing(q_uz_routed[x_ind,y_ind,:],q_uz_routed[x_ind,y_ind,0],sp_pars[x_ind,y_ind,10],sp_pars[x_ind,y_ind,11],p2[0]) 
##                        q=q_r
...
#_____________________________________________________________________________________________
"""
def distributed_new_model_struture2(lakecell,q_lake,DEM,flow_acc,flow_acc_plan, sp_prec, sp_et, sp_temp, sp_pars, p2, init_st=None, 
                ll_temp=None, q_0=None):
    '''
    Make spatially distributed HBV in the SM and UZ
    interacting cells 
...
def distributed_new_model_struture2_withoutlake(lakecell,q_lake,DEM,flow_acc,flow_acc_plan, sp_prec, sp_et, sp_temp, sp_pars, p2, init_st=None, 
                ll_temp=None, q_0=None):
    '''
    Make spatially distributed HBV in the SM and UZ
    interacting cells 
...
#    f = 1.0 - (np.nansum(a)/np.nansum(b))
    a=sum((q_rec-q_sim)**2)
    b=sum((q_rec-np.average(q_rec))**2)
    e=1-(a/b)
    return e
"""
#_____________________________________________________________________________________________
...
#    f = np.sqrt(np.nanmean(erro))
    f=np.sqrt(np.average((q_rec-q_sim)**2))
    return f
"""
#_____________________________________________________________________________________________
#_____________________________________________________________________________________________
