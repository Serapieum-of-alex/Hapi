...
#    assert history_fname[-4:] == ".txt", "history_fname should be txt file please change extension or add .txt ad the end of the history_fname"
    ...
    # initialize the optimizer & set the default options     
#    opt_engine = ALHSO(etol=0.0001,atol=0.0001,rtol=0.0001, stopiters=10,
#                       hmcr=0.5,par=0.5)
    opt_engine = ALHSO()
    
    for i in range(len(OptimizationArgs)):
        Optimizer.setOption(opt_engine, OptimizationArgs.keys()[i],
                            value=OptimizationArgs[OptimizationArgs.keys()[i]])
