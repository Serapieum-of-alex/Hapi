...
from bokeh.models.widgets import (Panel, Button, TextInput, Div, Tabs ,Slider,Select ,#, RadioGroup #DataTable, DateFormatter, TableColumn, 
...
def stations_coord_f_callback(attr,old,new):
    ...
    # get just the name of the file without the extension 
#    stations_coor_ds.data['file_name'][0]=stations_coor_ds.data['file_name'][0].split(".")[0]
    # write the file name
...
#    data=data.resample('H').sum()
    ...
    # get just the name of the file without the extension 
#    stations_values_ds.data['file_name'][0]=stations_values_ds.data['file_name'][0].split(".")[0]
...
#        layout_inputdata.children.append(messages_div)
#        print("length of time series between dates"+str(len(dates))+
#              " does not match with the length of the data in the file"+str(len(stations_values_ds.data['file_contents'][0])))
        
    #3- update the precipitation plot with the plot of the first station 
#    station_plot_ds = ColumnDataSource(dict(ds_time = dates, 
#                                            st_values = stations_values_ds.data['file_contents'][0][:,0]))
    ...
    layout_inputdata.children[2].children.append(row(cal_dist,gen_message))
#    cal_dist
...
gendata_slider= Slider(title="index ", start=0, end=0, value=0, step=1, )#title="Frame"
...
#stations_coor_ds.data['lat'][0]=[13.720999999999998,13.517999999999999,13.607000000000001,13.595999999999998,13.62,13.652999999999999,13.657,13.815]
#stations_coor_ds.data['long'][0]=[-88.926, -88.988, -88.86, -88.839, -89.01, -88.9, -88.861, -88.86200000000001]
#data=data.as_matrix()
#stations_values_ds.data['file_contents'][0]=data
...
def generate_dist_data():
    ...
    layout_inputdata.children[3].children.append(layout([gendata_plot],[row(gendata_slider,gendata_date)]))
#    layout_inputdata.children.append(gendata_plot)
#    layout_inputdata.children.append(row(gendata_slider,gendata_date))
...
layout_inputdata =layout(children=[
                    ...
                    [station_plot ],
#                    [column(cal_dist,gen_message)],
                    
                    ], sizing_mode="fixed")

                           
#layout_inputdata = layout([ [row1],#,p], #0
##                           [Time_step_in], #1
##                           [row([startdate_in,empty_div1,endtdate_in])], #2

#                           [stations_coord_upload_button,stations_coord_fname ], #3
#                           [stations_values_upload_button,stations_values_fname], #4
#                           [station_list], #5
#                           [station_plot ], #6
#                           [inputs_graph], #7
#                           [column(cal_dist,gen_message)],#8
##                    [tbl_bw_label],
##                    [data_table_bw] 
#                    ])
#
